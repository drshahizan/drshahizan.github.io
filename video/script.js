...
const competitionPhotos=['Pembukaan program','Sesi pembelajaran peserta','Aktiviti bengkel secara berkumpulan','Semangat peserta TVET','Kolaborasi antara peserta','Perkongsian dan perbincangan','Keceriaan sepanjang pertandingan','Kerja berpasukan pelajar','Sesi demonstrasi hasil','Penyampaian penghargaan','Pengiktirafan kepada peserta','Kenangan bersama peserta','Foto kumpulan program','Semangat warga kolej vokasional','Wajah ceria para peserta','Peserta bersedia menyertai aktiviti'];
const competitionGallery=$('#competitionGallery');
competitionPhotos.forEach((caption,i)=>{const n=i+1,button=document.createElement('button');button.className='competition-photo reveal';button.type='button';button.innerHTML=`<img loading="lazy" src="assets/images/galeri-kvp-${n}.png" alt="${caption} semasa Bengkel dan Pertandingan AI in Video Resume"><span>${caption}</span>`;button.onclick=()=>{$('#lightbox img').src=`assets/images/galeri-kvp-${n}.png`;$('#lightbox img').alt=`${caption} semasa Bengkel dan Pertandingan AI in Video Resume`;$('#lightbox p').textContent=`Sorotan pertandingan · ${caption}`;$('#lightbox').showModal()};competitionGallery.append(button);observer.observe(button)});
