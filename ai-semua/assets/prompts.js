...
const nav=document.getElementById('categoryNav'),root=document.getElementById('promptLibrary');categories.forEach((c,ci)=>{const link=document.createElement('a');link.href='#'+c.id;link.textContent=c.title;nav.appendChild(link);const sec=document.createElement('section');sec.className='prompt-category';sec.id=c.id;sec.innerHTML=`<div class="category-heading"><p class="kicker">Kategori ${ci+1}</p><h2>${c.title}</h2><p>${c.desc}</p></div><div class="library-grid">${c.prompts.map((p,i)=>`<article class="library-card"><h3>${i+1}. ${c.title.split(' & ')[0]}</h3><div class="prompt">${p}</div></article>`).join('')}</div>`;root.appendChild(sec)});
